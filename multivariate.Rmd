...
Some of these packages are familiar, but `{stringr}` manages the manipulation of strings, `{parallelDist}` exploits multiple processors/cores to efficiently calculate large dissimilarlty/distance matrices, and '{gplots}` calculates heat maps.  
...
The "plaid and blocks" pattern suggests that, as we might have expected, there is a longitudinal pattern in Oregon climates.
...
Get the discriminat scores (the values of the new discriminant function):
...
The loadings can be used to interpret or describe the discriminant functions. Here January temperature and January and annual precipitation are most highly correlated with the first discriminant function. Each of these have strong latitudinal gradient across the state: high in the west, low in the east. Notet that this gradient correspons well with a similar longitudinal gradient in cluster number.
...
The dissimilarity matrix (`dist_obs`) can be conveniently visualized by turning it into a `{terra}` SpatRaster, and then using the `{terra}` `plot()` function to visualize it. The rendering of the image is faster if the image is written to a file:
...
Use `ggplot2` to plot the matrix
...
```{r multivariate-45, echo=TRUE, eval=FALSE}
