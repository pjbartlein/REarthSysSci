...
```{r load packages2, messages=FALSE, results="hide", eval=FALSE, eval=FALSE}
library(maptools)
library(sp)
library(rgdal)
library(raster)
...
```{r read NE shapefiles2, eval=FALSE, eval=FALSE}
...
```{r read shapefiles, , results="hide", eval=FALSE, eval=FALSE}
...
```{r list data admin_poly, eval=FALSE}
head(admin0_poly)
```
...
```{r extract US and Candada, eval=FALSE}
...
```{r convert polygons to spatial lines, eval=FALSE}
...
```{r project shapefiles, eval=FALSE}
...
```{r set bounding box and trim, eval=FALSE}
...
```{r trim admin shapefiles, eval=FALSE}
...
```{r plot projected shapefiles, eval=FALSE}
...
```{r write coast shapes2, warning=FALSE, eval=FALSE}
...
```{r test shape2, fig.height=4, eval=FALSE}
...
```{r write other shapefiles2, results="hide", warning=FALSE, eval=FALSE}
# write out the various shapefiles 
outshape <- na10km_bb
...
```{r load packages again, results="hide", eval=FALSE}
library(sp)
```

Read the shapefiles:

```{r read projected shapefiles, results="hide", warning=FALSE, eval=FALSE}
...
```{r plot projected and trimmed shapefiles, eval=FALSE}
...
```{r read shade and shade colors, eval=FALSE}
...
```{r convert dataframe to spatial pixels, eval=FALSE}
...
```{r read shade color file, eval=FALSE}
...
```{r set color numbers, eval=FALSE}
...
```{r shaded relief map, echo=TRUE, eval=FALSE, messages=FALSE, eval=FALSE}
