...
plot(st_geometry(wus_pratio_sf), axes = TRUE, pch = 16, cex = 0.5, col = "gray")
```
...
```{r hd04, fig.height=6, fig.width=6, eval=FALSE}
# # a second map with some colors
ggplot() +
  geom_sf(data = na_sf, fill=NA, color="gray") +
...
ggplot(wus_states, aes(long, lat)) + geom_polygon(aes(group = group), color = "gray50", fill = "grey90") +
...
wus_sf <- st_as_sf(wus_shp, coords = c("lon", "lat"))
lon <- st_coordinates(wus_pratio_sf)[,1]
lat <- st_coordinates(wus_pratio_sf)[,2]
