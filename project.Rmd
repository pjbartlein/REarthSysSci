...
2. Copy the contents of [[plot\_alpha\_RScript.R]](https://pjbartlein.github.io/REarthSysSci/plot_alpha.html#plot_alpha_rscript.r-example-1) into it;
...
2. Copy the contents of [[plot\_alpha\_RMarkdown\_Notebook.Rmd]](https://pjbartlein.github.io/REarthSysSci/plot_alpha.html#plot_alpha_rmarkdown_notebook.rmd-example-2) into it;
1. Edit the paths in the script file as appropriate;
...
2. Copy the contents of [[plot\_alpha\_RMarkdown\_1Page.Rmd]](https://pjbartlein.github.io/REarthSysSci/plot_alpha.html#plot_alpha_rmarkdown_1page.rmd-example-3) into it;
1. Edit the paths in the script file as appropriate;
...
```{r yamlheader, echo=TRUE, eval=FALSE}
  ---
...
The second block of code includes some "global" code-chunk options (that apply to all "code chunks", or blocks of code set off by backtics (\`\`\`) (Remove `##`'s before using this code fragment)
...
## A multi-page R Markdown-generated website (Example 4) ##
...
1. Create a new R Markdown file in the projects folder (File > New File > R Markdown);
2. Delete the sample code in the new page; 
2. Copy the contents of [[plot\_alpha\_RMarkdown\_Site.Rmd]](https://pjbartlein.github.io/REarthSysSci/plot_alpha.html#plot_alpha_rmarkdown_site.rmd-example-4) into it;
1. Edit the paths in the script file as appropriate;
...
`index.Rmd`: (Remove `##`'s before attempting to run.)
...
`intro.Rmd`: (Remove `##`'s before attempting to run.)
...
`about.Rmd`:  (Remove `##`'s before attempting to run.)
...
"Configure Build Tools…"  A "Project Options" dialog will open.  On the "Project build tools:" dropdown box, select "Website"  The initial choices are probably fine, with the "Site directory" set to "(Project Root)" and the "Preview" and "Re-knit" checkboxes checked.  Upon clicking OK, a "Build" tab should appear in the upper-right pane.  If it doesn't, restart R Studio (Session > Restart R).  Select the build tab, and click on "Build Website".
